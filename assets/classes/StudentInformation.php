...
class StudentInformation
{
	...
	public function get_Information_by_Student_Id($Student_Id){
		
		$resultStudentInformation = array();
		
		
		
		$getStudentInformation = $this->_db->prepare("SELECT *
													FROM STUDENT AS t1
													WHERE t1.Student_Id=?;"
												);
		...
		$getStudentInformation->bindParam(1,$Student_Id);
		$getStudentInformation->execute();
		
			if($getStudentInformation->rowCount() != 0){
				$i = 0;
				
				
				while($result_Students = $getStudentInformation->fetch(PDO::FETCH_ASSOC)){
					...
					$College_1_Description	= (isset($result_Students['College_1_Description']) ? $result_Students['College_1_Description'] : null);
							...
							$resultStudentInformation[$i] = array(
				...
				return $resultStudentInformation;
			}else{
				return 0;
			}
