...
class Remaining
{
	...
	public function get_remaining_by_Student_ID($Student_Id){
		
		$resultRemaining = array();
		
		$getRemainingCourse = $this->_db->prepare("SELECT * FROM STUDENT AS t1, PROGRAM AS t2, PROGRAM_DETAIL AS t3, COURSE AS t4
													WHERE t1.Student_Id=?
													AND t1.Program_1=t2.Program_1 
													AND t2.ProgramID=t3.ProgramID 
													AND t3.isCourseRequired=1
													AND t3.CourseID=t4.CourseID
													AND t4.CourseNumber NOT IN(SELECT COURSE.CourseNumber FROM STUDENT, STUDENT_REGISTRATION, CLASS, COURSE
													WHERE STUDENT.Student_Id=t1.Student_Id
													AND STUDENT.Student_Id=STUDENT_REGISTRATION.Student_Id 
													AND STUDENT_REGISTRATION.ClassID=CLASS.ClassID
													AND CLASS.CourseID=COURSE.CourseID)
													ORDER BY t4.CourseNumber;");
															
															
		
		$getRemainingCourse->bindParam(1,$Student_Id);
		$getRemainingCourse->execute();
		
			if($getRemainingCourse->rowCount() != 0){
				$i = 0;
				
				
				while($result_Students = $getRemainingCourse->fetch(PDO::FETCH_ASSOC)){
